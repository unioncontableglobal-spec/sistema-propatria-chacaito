    ...
    <div className="p-6 bg-[#F8FAFC] min-h-screen">
      ...
      <div className="hidden print:block print-only text-black bg-white" style={{ width: '8.5in', minHeight: '11in', margin: '0 auto', padding: '0.5in', boxSizing: 'border-box', fontFamily: 'Arial, sans-serif' }}>
        ...
        <h1 className="text-center text-xl font-bold uppercase mb-4 tracking-wider border-b-2 border-black pb-2">
          Reporte de Recaudación (CxC) - {filtroMes || 'Histórico Total'}
        </h1>

        {/* Resumen */}
        <div className="flex justify-between mb-6 text-sm">
          <div>
            <p><strong>Meta de Recaudación:</strong> {formatUsd(metaCxc)}</p>
            <p><strong>Total Recaudado (USD):</strong> {formatUsd(kpis.totalUsd)}</p>
            <p><strong>Porcentaje Cobrado:</strong> {metaCxc > 0 ? ((kpis.totalUsd / metaCxc) * 100).toFixed(1) : 0}%</p>
          </div>
        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="border-b-2 border-black text-left">
              ...
              <th className="pb-1">Socio / Concepto</th>
            ...
            {filteredData.map((tx: any) => {
               ...
               return (
                 <tr key={tx.id} className="border-b border-gray-300">
                    ...
                    <td className="py-2">
                      <span className="font-bold">{tx.socio?.ficha}</span> - {tx.socio?.nombre_apellido}<br/>
